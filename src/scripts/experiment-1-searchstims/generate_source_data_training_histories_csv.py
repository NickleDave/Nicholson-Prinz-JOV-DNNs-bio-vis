...
def main(ckpt_root,
    ...
    # make list of dataframes, then use pd.concat to concatenate into one giant training history dataframe
    dfs = []
    for net_name in net_names:
        ...
        for method in methods:  # transfer learning? or train from randomly-initialized weights?
            ...
            for mode in modes:
                ...
                if len(this_mode_ckpt_roots) != 1:
                    raise ValueError(
                        "did not find only a single checkpoint root "
                        f"for net '{net_name}', method '{method}', and mode '{mode}'."
                        f"Instead found:\n{this_mode_ckpt_roots}"
                    )
                this_ckpt_root = this_mode_ckpt_roots[0]
    ...
    # finally, save csvs
    dfs = pd.concat(dfs)
    dfs.to_csv(source_data_root.joinpath(csv_filename), index=False)
...
CHECKPOINT_ROOT = SEARCHSTIMS_ROOT.joinpath('checkpoints/3stims')
